...
string at22base = "https://platform.at22.altinn.cloud/resourceregistry/api/v1/";
...
for (int i = 0; i < resourceList.Count; i++)
{
    ...
    if (resource.ResourceType.Equals(ResourceType.CorrespondenceService))
    {
        await resourceRegistry.ReloadResourceSubects(resource.Identifier);
    }

    if (resource.ResourceType.Equals(ResourceType.BrokerService))
    {
        await resourceRegistry.ReloadResourceSubects(resource.Identifier);
    }

    if (resource.ResourceType.Equals(ResourceType.MaskinportenSchema))
    {
        await resourceRegistry.ReloadResourceSubects(resource.Identifier);
    }
