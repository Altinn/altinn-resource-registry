...
namespace Altinn.ResourceRegistry.Core.Services
{
    ...
    public class ResourceRegistryService : IResourceRegistry
    {
        ...
        {
            List<ServiceResource> resourceList = await GetResourceList(includeApps: false, includeAltinn2: false, includeExpired: false, includeMigratedResources: false, cancellationToken);
        ...
        /// <inheritdoc />
        public async Task<List<ServiceResource>> GetResourceList(bool includeApps, bool includeAltinn2, bool includeExpired, bool includeMigratedResources, CancellationToken cancellationToken = default)
        {
            ...
            if (includeApps || includeAltinn2)
            {
                ...
                if (includeApps)
                {
                    tasks.Add(GetAltinn3Applications(includeMigratedResources, cancellationToken));
                }
            }
            ...
            async Task<List<ServiceResource>> GetAltinn3Applications(bool includeMigratedResources, CancellationToken cancellationToken = default)
            {
                ApplicationList applicationList = await _applicationsClient.GetApplicationList(includeMigratedResources, cancellationToken);
